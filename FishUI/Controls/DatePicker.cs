...
namespace FishUI.Controls
{
    ...
    public class DatePicker : Control, IFishUIDebugSnapshotProvider
    {
        ...
        private bool _alwaysOnTopBeforeOpen;
        ...
        public void Open()
        {
            ...
            if (!wasOpen) _alwaysOnTopBeforeOpen = AlwaysOnTop;
            DateTime oldDisplayMonth = _displayMonth;
            _isOpen = true;
            _displayMonth = new DateTime(_value.Year, _value.Month, 1);
            if (!wasOpen)
        ...
        public void Close()
        {
            ...
            _isOpen = false;
            _hoveredDay = -1;
            if (wasOpen) AlwaysOnTop = _alwaysOnTopBeforeOpen;
            FishUI?.UnregisterOverlay(this);
            if (wasOpen)
        ...
        internal void DrawCalendarPopupOverlay(FishUI UI)
        {
            if (!_isOpen) return;
            DrawCalendarPopup(UI, GetAbsolutePosition(), GetAbsoluteSize(), UI.Settings.FontDefault);
        }

        protected override void OnAttachedToFishUI(FishUI ui)
        {
            if (_isOpen) ui.RegisterOverlay(this);
        }

        protected override void OnDetachedFromFishUI(FishUI ui)
        {
            if (_isOpen) Close();
            ui.UnregisterOverlay(this);
        }
