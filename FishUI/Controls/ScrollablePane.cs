...
namespace FishUI.Controls
{
    ...
    public class ScrollablePane : Control, IFishUIDebugSnapshotProvider
    {
        ...
        {
            Vector2 content = GetEffectiveContentSize(); Vector2 viewport = Size;
            writer.Write("viewportSizeLogical", FishUIDebugPoint.From(viewport));
            writer.Write("contentSizeLogical", FishUIDebugPoint.From(content));
            writer.Write("scrollOffsetLogical", FishUIDebugPoint.From(ScrollOffset));
            writer.Write("maximumOffsetLogical", new FishUIDebugPoint(Math.Max(0, content.X - viewport.X), Math.Max(0, content.Y - viewport.Y)));
        }
        ...
        {
            Vector2 absPos = GetAbsolutePosition();
            Vector2 visibleArea = Scale(GetVisibleArea());

            // Draw content children with scissor to visible area (not full control bounds)
            using (UI.Graphics.PushScissorScope(absPos, visibleArea))
            {
                Control[] children = FrameChildrenPaintOrder;
                for (int i = 0; i < children.Length; i++)
                {
                    Control child = children[i];
                    if (!child.Visible || child == _scrollBarV || child == _scrollBarH)
                        continue;
        ...
        public Vector2 GetScrollAdjustedChildPosition(Control child)
        {
            ...
            return GetAbsolutePosition() + Scale(childRelPos - ScrollOffset);
        }
