...
namespace FishUI.Controls
{
    ...
    public partial class TreeView : Control
    {
        ...
        {
            _visibleNodes.Clear();
            _visibleTraversalVisited.Clear();
            float y = 0;

            if (ShowRootNodes)
            {
                for (int i = 0; i < Nodes.Count; i++) CollectVisibleNodes(Nodes[i], ref y, 0);
            }
            else
            {
                for (int i = 0; i < Nodes.Count; i++)
                    for (int childIndex = 0; childIndex < Nodes[i].Children.Count; childIndex++)
                        CollectVisibleNodes(Nodes[i].Children[childIndex], ref y, -1);
            }
        }

        private readonly HashSet<TreeNode> _visibleTraversalVisited = new HashSet<TreeNode>();

        private void CollectVisibleNodes(TreeNode node, ref float y, int baseDepth)
        {
            if (node == null || !_visibleTraversalVisited.Add(node)) return;
        ...
        public override void DrawControl(FishUI UI, float Dt, float Time)
        {
            ...
            using FishUIScissorScope clip = UI.Graphics.PushScissorScope(contentPos, contentSize);
