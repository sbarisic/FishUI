...
public sealed class DiagnosticSnapshotTests
{
    ...
    public void DiagnosticHotkeyIsInactiveWhenDisabledAndCapturesFollowingDrawWhenEnabled()
    {
        using (var disabled = new FishUITestFixture())
        {
            ...
            disabled.Update();
            Assert.Equal(3, sink.KeyPressCount);
    ...
    public async Task TreeNodeIdentityIsStableAndCyclesAndDuplicatesAreBounded()
    {
        ...
        fixture.Update();
        long firstId = (long)Assert.Single((await firstTask).Controls,
            item => item.Type == nameof(TreeView)).ControlData!["selectedNodeId"];
        ...
        fixture.Update();
        FishUIDebugSnapshot second = await secondTask;
        FishUIControlSnapshot state = Assert.Single(second.Controls, item => item.Type == nameof(TreeView));
    ...
    public async Task ConsumptiveInputIsPolledOnlyByNormalInputFlow()
    {
        ...
        Assert.Equal(2, fixture.Input.GetKeyPressedCallCount);
    ...
    public async Task DiagnosticDragEventsRequireThresholdMovement()
    {
        ...
        fixture.Update();
        fixture.Input.SimulateMouseMove(new Vector2(30, 25));
        fixture.Update();
        fixture.Input.SimulateMouseUp(FishMouseButton.Left);
        Task<FishUIDebugSnapshot> capture = FishUIDiagnostics.CaptureAsync(fixture.UI, StructuredOptions());
        fixture.Update();
        ...
        Assert.Contains("DragUpdated", snapshot.InteractionSummary);
        Assert.Contains("samples=2", snapshot.InteractionSummary);
        Assert.Contains("totalDelta=(10,5)", snapshot.InteractionSummary);
    }

    [Fact]
    public async Task LineChartReportsManualAndEffectiveViewRanges()
    {
        using var fixture = new FishUITestFixture();
        ...
        chart.Pause();
        fixture.UI.AddControl(chart);
        Task<FishUIDebugSnapshot> capture = FishUIDiagnostics.CaptureAsync(fixture.UI, StructuredOptions());

        fixture.Update();
        FishUIDebugSnapshot snapshot = await capture;
        FishUIControlSnapshot state = Assert.Single(snapshot.Controls, control => control.Type == nameof(LineChart));

        Assert.Equal(3f, state.ControlData!["manualViewStart"]);
        Assert.Equal(8f, state.ControlData["manualViewEnd"]);
        Assert.Equal(15f, state.ControlData["effectiveViewStart"]);
        Assert.Equal(20f, state.ControlData["effectiveViewEnd"]);
    }
    ...
    {
        FishUI.FishUI ui = new FishUI.FishUI(new FishUISettings(), graphics, new MockFishUIInput(), new MockFishUIEvents(), new MockFishUIFileSystem())
        {
            Width = 4,
            Height = 4
        };
