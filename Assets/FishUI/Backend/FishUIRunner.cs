...
public class FishUIRunner : MonoBehaviour
{
	FishUI.FishUI FUI;
	FishInput fishInput;
	FishGfx fishGfx;
	...
	void CreateRenderer()
	{
		FishFS FS = new FishFS();
		fishGfx = new FishGfx(FS);
		fishInput = new FishInput();
		FishUISettings Settings = new FishUISettings();
		FishEvents Events = new FishEvents();

		FUI = new FishUI.FishUI(Settings, fishGfx, fishInput, Events, FS);
		FUI.Init();
	...
	void Update()
	{
		...
		FUI.TickUpdate(Dt, CurrentTime);
	}

	// OnGUI is called for rendering and handling GUI events - required for IMGUI rendering
	void OnGUI()
	{
		if (fishGfx == null || FUI == null)
			return;

		// Reset clip state at start of frame
		fishGfx.BeginFrame();

		float Dt = UnityEngine.Time.deltaTime;
		float CurrentTime = UnityEngine.Time.time;
		FUI.TickDraw(Dt, CurrentTime);

		// Ensure all clips are closed at end of frame
		fishGfx.EndFrame();
	}
